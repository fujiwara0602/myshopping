...
try {
    ...
    if ($existingCartItem) {
        ...
        $stmt = $pdo->prepare('UPDATE cart_product SET number = number + :quantity WHERE id = :id');
        $stmt->bindParam(':quantity', $quantity, PDO::PARAM_INT);
        $stmt->bindParam(':id', $existingCartItem['id'], PDO::PARAM_INT);
        $stmt->execute();
    } else {
        // 商品がカート内に存在しない場合、新しいcart_productを作成
        $stmt = $pdo->prepare('INSERT INTO cart_product (cart_id, product_id, number) VALUES (:cart_id, :product_id, :number)');
        $stmt->bindParam(':cart_id', $cartId, PDO::PARAM_INT);
        $stmt->bindParam(':product_id', $productId, PDO::PARAM_INT);
        $stmt->bindParam(':number', $quantity, PDO::PARAM_INT);
        $stmt->execute();
    }
